...
def multi_thread_method(set_name,N_slice):
                        ...
                        if (np.sum([img_temp == 200]) == 0) & (np.sum([img_temp > 1]) >= 0.25 * N_pixel):
                            for slice_id2 in search_list:
                                ...
                                for time_diff in time_diff_list:
                                    ...
                                    if ((T_id2 > 0) & (T_id2 < 16)):
                                        img2, N_row, N_col = read_slice(input_file, slice_size, slice_id2, T_id2,
                                                                        H_id)
    ...
    match_all_pd = pd.DataFrame(match_all,
                                columns=['slice_id1', 'row_id1', 'col_id1', 'T_id1', 'slice_id2', 'row_id2',
                                         'col_id2', 'T_id2'])
    ...
    for ind, value in Pano_time_pd.iterrows():
        ...
        slices = match_all_pd[
            (match_all_pd.slice_id1.isin(slice_range)) & (match_all_pd.slice_id2.isin(slice_range))]
